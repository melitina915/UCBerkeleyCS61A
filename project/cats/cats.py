...
def about(topic):
    ...
    def me(p):
        for word in topic:
            for wor in split(remove_punctuation(p)):
                if lower(word) == lower(wor):
                    return True
        return False
    return me
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    lowdiff, lowword = limit, typed_word
    for word in word_list:
        diff = diff_function(typed_word, word, limit)
        if diff == lowdiff:
            lowdiff, lowword = diff, word
            break
    for word in word_list:
        if typed_word == word:
            return word
...
def feline_fixes(typed, reference, limit):
    ...
    def swap_helper(i, left):
        if i < len(typed) and i < len(reference) and left >= 0:
            if typed[i] != reference[i]:
                return 1 + swap_helper(i+1, left-1)
            else:
                return swap_helper(i+1, left)
        else:
...
def hidden_kittens(typed, reference, limit):
    ...
    def helper(typed, reference, limit):
        if (len(typed)==0 and len(reference)==0) or len(reference)==0:
            return 1
        if len(typed)==0:
            return 0
        if typed[len(typed)-1] == reference[len(reference)-1]:
            return helper(typed[:-1], reference[:-1], limit)+helper(typed[:-1], reference, limit)
        else:
...
def time_per_word(words, times_per_player):
    ...
    t = []
    l = len(times_per_player)
    for i in range(l):
        time = [1] * (len(times_per_player[i]) - 1)
        for j in range(1, len(times_per_player[i])):
            time[j - 1] = times_per_player[i][j] - times_per_player[i][j - 1]
        t.append(time)
...
def fastest_words(match):
    ...
    for k in word_indices:
        min = time(match, 0, k)
        for player in player_indices:
            if time(match, player, k) < min:
                min = time(match, player, k)
                flag = player
        fast = get_word(match, k)
        lst[flag].append(fast)
        flag = 0
    return lst
