...
def oink_points(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        return max(2*(opponent_score//10 - opponent_score//100 * 10) - opponent_score%10, 1)
...
def pigs_on_prime(player_score, opponent_score):
    ...
    if is_prime(player_score) == False:
        return 0
    elif player_score == 1 or player_score == 2:
        return 1
    else:
        num = 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if next_player(who) == 1:
            ...
            score0 += take_turn(num_rolls, score0, score1, dice, goal=GOAL_SCORE)
            score0 = score0 + pigs_on_prime(score0, score1)
        elif next_player(who) == 0:
            num_rolls = strategy1(score1, score0)
            score1 += take_turn(num_rolls, score1, score0, dice, goal=GOAL_SCORE)
    ...
    # BEGIN PROBLEM 7
        play, mess = say(score0, score1, leader)
        #print(play, score0, score1, who, leader)
        if mess != None and who != play:
            print(mess)
        leader = play
    # END PROBLEM 7
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        leader = 0
        message = "Player " + str(leader) + " takes the lead by " + str(score0 - score1)
    elif score1 > score0:
        leader = 1
        message = "Player " + str(leader) + " takes the lead by " + str(score1 - score0)
    else:
        leader = None
        message = None
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def average(*args):  # *args is the formal parameter of function fn as well
        total, i = 0, 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i, score = 1, 0
    while i <= 10:
        avedice = make_averaged(roll_dice, total_samples)
        maxscore = avedice(i, dice)
        if maxscore > score:
            score = maxscore
            num = i
        i += 1
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if oink_points(score, opponent_score) > 0:
        score = oink_points(score, opponent_score) + score
    if pigs_on_prime(score, opponent_score) > 0 or oink_points(score, opponent_score) + pigs_on_prime(score, opponent_score) >= threshold:
        return 0
    else:
        return num_rolls
    #return 6  Remove this line once implemented.
