...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        pl = self.place
        trans = 0
        while pl.is_hive == False:
            if random_bee(pl.bees) != None:
                if trans in range(self.lower_bound, self.upper_bound+1):
                    return random_bee(pl.bees)
                else:
                    pl = pl.entrance
                    trans += 1
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if cls.food_cost > gamestate.food:
            print('Not enough food remains to place ' + cls.__name__)
            return
        if not gamestate.yesqueen:
            gamestate.yesqueen = True
            ...
            while behind:
                if behind.ant:
                    ...
                    if behind.ant.is_container and behind.ant.ant_contained:
                        if not behind.ant.ant_contained.doubled:
                            behind.ant.ant_contained.damage *= 2
                            behind.ant.ant_contained.doubled = True
                behind = behind.exit
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scary:
            ...
            self.scareturn = self.scareturn - 1

        if self.slowy:
            self.slowturn = self.slowturn - 1
            if self.slowturn == 0:
                self.slowy = False
            if gamestate.time % 2 == 0 and self.health > 0 and destination:
    ...
    def slow(self, length):
        ...
        "*** YOUR CODE HERE ***"
        self.slowy = True
        self.slowturn = self.slowturn + length
        # END Problem EC
    ...
    def scare(self, length):
        ...
        if self.scared:
            ...
            self.scary = True
            self.scareturn = self.scareturn + length
            self.scared = True
        # END Problem EC
