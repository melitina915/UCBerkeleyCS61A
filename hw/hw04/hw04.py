...
class SmartFridge:
    ...
    def use_item(self, item, quantity):
        "*** YOUR CODE HERE ***"
        self.items[item] = self.items[item] - min(quantity, self.items[item])
...
class VendingMachine:
    ...
    def add_funds(self, amount):
        ...
        self.bal += amount
        return 'Current balance: ${0}'.format(self.bal)

    def vend(self):
        if self.num == 0 and self.bal == 0:
            return 'Nothing left to vend. Please restock.'

        change = self.bal - self.price
        if change < 0:
            return 'Please update your balance with ${0} more funds.'.format(-change)
        elif change > 0:
            self.bal = 0
            self.num -= 1
